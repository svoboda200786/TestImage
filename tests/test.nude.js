...
var global_nude, myresponse, myheaders;
(function(){
		...
		loadImageById = function(id){
			...
			loadImage(id).then(img => {
				...
				nude.scan(function(result){
					...
					// fs.writeFile(__dirname + "/images/1.jpg", buffer, function (){});
					myresponse.writeHead(200, myheaders);
      		myresponse.end(text);  
				});			
			})
		},
...
function renderAll(response, side, headers){
	myresponse = response;
	myheaders = headers;
	global_nude.load(side);
	// 	global_nude.load(__dirname + "/images/" + side);
}
